...
export default function JobFilters() {
  ...
  const [maxSalary, setMaxSalary] = useState(80);
  ...
  const handleMouseDown = (type) => {
    const move = (e) => {
      ...
      if (type === 'min') {
        setMinSalary(Math.min(Math.max(0, percent), maxSalary - 5));
      } else {
        setMaxSalary(Math.max(Math.min(100, percent), minSalary + 5));
      }
    };
    ...
    <div className="bg-white rounded-xl shadow-md p-6 mb-8">
      <div className="flex flex-wrap md:flex-nowrap items-center justify-between divide-x divide-gray-300">
        ...
        <div className="flex items-center flex-1 min-w-[200px] px-6">
          ...
          <span className="text-sm text-gray-500 mr-2 whitespace-nowrap">Preferred Location</span>
          <select className="border border-gray-300 px-3 py-2 text-sm rounded-md text-gray-700 focus:outline-none w-full">
  ...
  />
          ...
          <span className="text-sm text-gray-500 mr-2 whitespace-nowrap">Job Type</span>
          <select className="border border-gray-300 px-3 py-2 text-sm rounded-md text-gray-700 focus:outline-none w-full">
        ...
        <div className="flex flex-col flex-1 min-w-[200px] pl-6">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-sm text-gray-500">₹{Math.round(minSalary)}K - ₹{Math.round(maxSalary)}K</span>
          </div>
          <div
            ref={sliderRef}
            className="relative w-full h-1 bg-gray-800 rounded cursor-pointer"
          >
            <div
            ...
            ></div>
            <div
              className="absolute w-4 h-4 bg-indigo-500 rounded-full top-1/2 transform -translate-y-1/2"
              style={{ left: `${minSalary}%` }}
              onMouseDown={() => handleMouseDown('min')}
            ></div>
            <div
              className="absolute w-4 h-4 bg-indigo-500 rounded-full top-1/2 transform -translate-y-1/2"
              style={{ left: `${maxSalary}%` }}
